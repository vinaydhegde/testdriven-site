...
1. "Image": `046505967931.dkr.ecr.us-east-1.amazonaws.com/flask-microservices-users:staging`
...
##### Container 2: *flask-microservices-users-db*

1. "Container name": `users-db`
1. "Image": `046505967931.dkr.ecr.us-east-1.amazonaws.com/flask-microservices-users_db:staging`
1. "Memory Limits (MB)": `300` soft limit
...
Next, grab the Container ID for the *flask-microservices-users*, enter the Shell within the running container, and then update the database:
...
Then, update the `SECRET_KEY` environment variables for `staging` in *docker_push.sh*, to match the Task Definition:
...
Finally, ensure [http://EC2_PUBLIC_IP/users](http://EC2_PUBLIC_IP/users) works as expected.
